...
from covercore.apps.quote.models import GeneralLiabilityQuoteRequest
from covercore.apps.quote.service.hartford_client import CoverCoreHartfordClient


class CreateGeneralLiabilityQuoteRequestResourceAction(CreateModelResourceAction[GeneralLiabilityQuoteRequest]):

    def _save_form(self, form: LazyModelForm, request: Dict[str, Any], account_member: Optional[ApiAccountMember],
                   resource: GeneralLiabilityQuoteRequest) -> GeneralLiabilityQuoteRequest:
        ...
        service.send_general_liability_quote_request(obj)
        return obj


class GeneralLiabilityQuoteRequestCollectionApiView(ModelCollectionApiView[GeneralLiabilityQuoteRequest]):
    require_authentication = True

    def _get_action_configuration(self) -> Dict[HttpMethod, ResourceAction]:
        config = super()._get_action_configuration()
        config[HttpMethod.POST] = CreateGeneralLiabilityQuoteRequestResourceAction(self._get_model())
        return config

    def _get_model(self) -> Type[GeneralLiabilityQuoteRequest]:
        return GeneralLiabilityQuoteRequest

    def _get_queryset(self) -> 'QuerySet[GeneralLiabilityQuoteRequest]':
        return self.authenticated_account_member.api_account.general_liability_quote_requests.all()
